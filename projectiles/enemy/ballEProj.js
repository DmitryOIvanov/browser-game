...
export default class BallEProj {
    ...
    isOutbound() {
        if (this.vx >= 0 && this.x > playField.x + this.radius) return true;
        if (this.vx <= 0 && this.x < - this.radius) return true;
        if (this.vy >= 0 && this.y > playField.y + this.radius) return true;
        if (this.vy <= 0 && this.y < - this.radius) return true;
        return false;
    }
    ...
    timestep(amount) {
        ...
        if (this.isOutbound()) this.retired = true;
