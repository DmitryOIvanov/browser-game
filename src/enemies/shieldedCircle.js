...
export default class ShieldedCircle extends AbstractEnemy {
    ...
    destroySegment(segID) {
        ...
        if (segID == NUM_SEGMENTS) {
            playField.addParticle(new ExplodingRingParticle(this.x, this.y, 1.5 * IN_RAD, 2.5 * IN_RAD, 6, FlatColor.WHITE));
        } else {
            const angle = 2 * Math.PI * (segID + 0.5) / NUM_SEGMENTS + this.rot;
            const dist = 0.5 * (MID_RAD + OUT_RAD);
            playField.addParticle(new ExplodingRingParticle(this.x + dist * Math.cos(angle), this.y + dist * Math.sin(angle), 20, 35, 6, FlatColor.WHITE));
        }
